...
def yml2cs(actions:list[dict[str,dict[str,str]]], namespace:str, outer_class_name:str):
    ...
    for action in actions:
        ...
        for field in request:
            request_fields.append(FILED_PATTERN.replace('{T}',request[field]).replace('{field_name}',_(field)))
        request_fields_str = '\n'.join(request_fields)
        request_fields_str = f'\n{request_fields_str}\n' if request_fields else request_fields_str
        produce_req_expr = PRODUCE_EMPTY_REQ_EXPR_PATTERN if not request_fields else PRODUCE_REQ_EXPR_PATTERN
        ...
        for field in response:
            response_fields.append(FILED_PATTERN.replace('{T}',response[field]).replace('{field_name}',_(field)))
        ...
        for field in response:
            ...
            produce_resp_expr_contents.append(RESP_JOBJECT_ENTRY_PATTERN.replace('{arg_name}',_(field,False)))
