    ...
    public partial class CreateMinecraftFabricInstanceProvider : ICreateInstanceProvider
    {
        ...
        private async void FinishSetup(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                bool useMirror = SettingsManager.Get?.InstanceCreation?.UseMirrorForMinecraftFabricInstall ?? false;
                string serverJarUrl = useMirror
                ...
                string serverFileName = $"fabric-server-mc.{mcVersion}-loader.{fabricVersion}-launcher.{mcVersion}.jar";

                var confirmationMessage = Lang.Tr["CreateInstanceConfirmationMessage"] +
                                          $"{Lang.Tr["InstanceName"]}: {instanceName}\n" +
                                          $"{Lang.Tr["InstanceType"]}: {Lang.Tr["InstanceType_MCFabric"]}\n" +
                                          $"{Lang.Tr["MinecraftVersionLabel"]}: {mcVersion}\n" +
                                          $"{Lang.Tr["FabricLoaderVersionLabel"]}: {fabricVersion}\n" +
                ...
                var request = CreateRequest(
                    instanceName, serverJarUrl, serverFileName, javaPath, arguments, mcVersion,
