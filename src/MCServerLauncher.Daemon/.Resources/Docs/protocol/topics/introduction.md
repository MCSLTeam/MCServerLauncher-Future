...
协议的唯一机器可读运行时来源是启动后冻结的 typed catalog。连接成功后调用 `rpc.discover` 可取得当前 runtime OpenRPC 文档；HTTP `GET /apifox.json` 由同一 frozen catalog 生成（含已准入插件）。仓库内 checked-in Apifox 基线仅覆盖内置协议，供 CI `--check` 使用。
