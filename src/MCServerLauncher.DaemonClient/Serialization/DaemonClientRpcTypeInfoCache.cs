...
/// <summary>
///     Typed cache for daemonclient RPC boundary <see cref="JsonTypeInfo{T}" /> lookups.
///     Resolves exclusively from Common + DaemonClient source-generated contexts (no reflection fallback).
...
/// </summary>
internal static class DaemonClientRpcTypeInfoCache<T>
{
    ...
    public static JsonTypeInfo<T> TypeInfo =>
        _cachedTypeInfo ?? throw new NotSupportedException(
            $"DaemonClient RPC type-info cache does not provide source-generated JsonTypeInfo for {typeof(T).FullName}. "
            + $"Type must be registered in Common or DaemonClient serializer contexts.");
    ...
    private static JsonTypeInfo<T>? TryResolveSourceGen()
    {
        try
        {
            ...
            var options = DaemonClientRpcJsonBoundary.SourceGenStjOptions;
