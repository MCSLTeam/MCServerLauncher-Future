...
| `2.0.0-preview.6` | Not published. Preview-2 internal baseline, same local-feed consumption. One payload differs from `.5`: `MCServerLauncher.Common`, because the automation trigger and action vocabulary was completed in it. The other six packed entries are byte-identical. Fingerprints: `docs/preview2-package-pin.md`, machine-checked by `PinnedPayloadHashesMatchTheAcceptanceRecord`. |
