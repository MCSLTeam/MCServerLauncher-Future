...
>
> **Plan status:** Architecture approved. Phase 0 through Phase 3, Phase 4A daemon V2, and Phase 4B daemon-client/WPF cutover are complete and independently reviewed. Phase 4C is active: M1 sole `/api/v2` authority, M2a canonical DTO/persistence-boundary migration, and their reviews are complete; M2b legacy `Error` migration is next. Phase 4C and Phase 4 overall remain incomplete and unreleasable; residual V1 daemon/Common/client/generator source and daemon-internal `Error` paths are intentionally retained for M2b-M4.
>
