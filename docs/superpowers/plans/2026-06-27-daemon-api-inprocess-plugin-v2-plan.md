...
- [x] 实现 §8.3 catalog full snapshot/delta mirror；覆盖 subscribe-before-read race、version gap、duplicate conflict 与 reconnect resync。
...
2. **4B client cutover（complete）:** DaemonClient remote application implementation 与 WPF typed call sites 全部通过；DaemonClient 使用 `StatePublisher<InstanceCatalogSnapshot>` 维护 remote mirror。
3. **4C deletion proof:** 切换唯一 endpoint，删除 V1/Common/client/generator 路径并通过 inventory search、full tests 与 build。

**4A sub-exit:** Phase 4A daemon V2 completed across the 11 commits `b394957a^..98f07298`. Final acceptance passed Daemon.ApiTests 56/56, the real TouchSocket host test 1/1, and Release ProtocolTests plus the final Release `--no-build` gate at 782/782. The recorded daemon, full-solution, and benchmark Release builds each emitted 0 warnings / 0 errors; ProtocolDocs `--check` matched catalog hash `74268afdb2bb3dd9be54cc1edf92017f581d305270f6be201f2c033eb2f4b44f`; `git diff --check` passed. Independent Sol max final review reported 0 open P0/P1/P2. The pre-existing ProtocolTests source `CS0105` duplicate-using warning remains separately tracked and is not a Phase 4A regression. Paired V2/V1 performance validation remains a Phase 6 residual. `/api/v2` is production-composed through TouchSocket, while `/api/v1` intentionally remains a branch-internal migration path until 4C; Phase 4B is now complete, but Phase 4C remains pending, so this sub-exit is not a releasable Phase 4 or product completion.

**4B sub-exit:** Phase 4B daemon-client remote application/catalog mirror and the WPF connection-layer consumer cutover are complete and independently reviewed. The final independent Sol max review reported `P0=0 / P1=0 / P2=0`. Acceptance passed the Windows-targeted WPF tests at 19/19 and Release ProtocolTests `--no-build` at 1105/1105; the recorded DaemonClient, WPF, and full-solution Release builds each emitted 0 warnings / 0 errors; and `git diff --check` passed. The WPF V1/raw-symbol search was clean except for `EventRuleEditorDialog`'s UI/domain string property named `ActionType`, which is not a protocol/runtime reference. This WPF scope proves connection-layer consumers use the typed application/event boundary and localized error mapping; it does not claim an unperformed manual UI smoke test or runtime end-to-end UI exercise. `/api/v1` and the V1 daemon/Common/daemon-client/generator deletion targets remain for Phase 4C, so Phase 4 remains incomplete and unreleasable.

**4C M1 sub-exit (2026-07-14):** The sole `/api/v2` production authority and V2 connection administration are complete and independently reviewed. Sol max review reported `P0=0 / P1=0 / P2=0`. Acceptance passed the daemon Release build and full-solution Release build with 0 warnings / 0 errors, Release ProtocolTests `--no-build` at 1110/1110, production-authority residual search at 0 matches, and `git diff --check`. Existing focused evidence includes real-host `/api/v1` rejection and HTTP V2 metadata checks; no manual runtime smoke is claimed here. M2-M4 retain the legacy V1 daemon/Common/client/generator source, DTO/error/persistence-resolver decontamination, and physical deletion work, so Phase 4C and Phase 4 overall remain incomplete and unreleasable.
...
- [x] pack Daemon.API；运行 package validation 与 dependency graph checks。
- [x] 更新 third-party notices/license inventory，记录 MessagePipe 1.8.2 与 daemon-internal NuGet.Versioning。
...
dotnet publish tests/Fixtures/Plugins/ReturnedError/ReturnedError.csproj -c Release -o artifacts/plugin-e2e/daemon/plugins/fixture.returned-error
dotnet publish tests/Fixtures/Plugins/Throwing/Throwing.csproj -c Release -o artifacts/plugin-e2e/daemon/plugins/fixture.throwing
...
- 2026-07-14: Completed and independently reviewed Phase 4B daemon-client remote application/catalog mirror plus the WPF connection-layer consumer cutover with final `P0=0 / P1=0 / P2=0`. The cutover centralized typed restart composition, removed WPF transport-serializer coupling, replaced V1 WPF call sites with typed application/event paths and localized errors, and added Windows-targeted WPF coverage. Acceptance passed WPF tests 19/19, Release ProtocolTests `--no-build` 1105/1105, DaemonClient/WPF/full-solution Release builds at 0 warnings / 0 errors, the WPF V1/raw-symbol search with only `EventRuleEditorDialog`'s UI/domain string `ActionType` remaining, and `git diff --check`. Phase 4C sole-endpoint switch and V1 runtime/Common/client/generator deletion remain pending, so release-atomic Phase 4 is still incomplete and unreleasable.
- 2026-07-14: Phase 4C M1 sole `/api/v2` authority and V2 connection administration completed and independently reviewed with `P0=0 / P1=0 / P2=0`. Acceptance passed daemon/full-solution Release builds at 0 warnings / 0 errors, Release ProtocolTests `--no-build` at 1110/1110, production-authority residual search at 0 matches, and `git diff --check`; existing focused evidence covers real-host `/api/v1` rejection and HTTP V2 metadata. Legacy V1 daemon/Common/client/generator source and M2 legacy DTO/error/persistence-resolver decontamination remain for M2-M4; Phase 4C and Phase 4 overall are still incomplete and unreleasable.
