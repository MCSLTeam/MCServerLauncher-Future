...
A bundle loads only when the daemon `config.json` opts it in explicitly: `plugins.entries.<plugin-id>.enabled` must be `true`. A plugin id absent from `plugins.entries` is disabled. Required features must additionally fall within the effective grant set (`grant_level` plus `plugins.plugin_grants.<plugin-id>`); features above the grant level need an interactive approval or a recorded `plugins.admissions` decision. If a manifest declares `dependencies.version: 1` with `dependencies.plugins`, those dependency ids and NuGet-style version ranges are checked after preflight. If it declares `dependencies.contracts`, each Contracts DLL must be present in the bundle, match its assembly identity, satisfy the declared version range, and match the declared SHA-256 fingerprint. Missing, incompatible, disabled, skipped, cyclic, conflicting, or failed dependencies skip the affected consumer set without loading its entry assembly.

## Startup and shutdown

Plugins are discovered, validated, dependency-sorted, configured, started, admitted to the frozen catalog, and activated before the first client connection is accepted. Providers start before consumers, and shutdown reverses that order so consumers stop before providers. Declared Contracts assemblies are loaded once into shared contract load contexts and resolved into participating plugin ALCs so provider/consumer interfaces keep one type identity. Direct typed imports are available only for declared plugin dependencies, and plugin-facing event subscriptions are tied to plugin lifetime ownership. A failed plugin is logged and skipped without preventing daemon startup; consumers whose provider failed do not run their own `StartAsync`. Shutdown cancels plugin lifetimes, stops successful plugins in reverse start order, closes event/RPC ownership, and then stops the daemon services.
